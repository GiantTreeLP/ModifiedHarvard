...
<xsl:stylesheet version="1.0" 
	...
	<xsl:template name ="DisplayPageOrPages">
		...
		<xsl:choose>
			<xsl:when test="contains($pages, '-')">
				<xsl:value-of select="concat('Seiten ', $pages)"/>
			</xsl:when>
			<xsl:when test="contains($pages, ',')">
				<xsl:value-of select="concat('Seiten ', $pages)"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="concat('Seite ', $pages)"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="BibDisplayAuthorPatent">
		<xsl:param name ="DisplayLastNameOnly"/>
		...
		<xsl:choose>
			...
			<xsl:when test="$cInventors>0 and 5>$cInventors">
				<xsl:for-each select="b:Author/b:Inventor/b:NameList/b:Person">
					...
					<xsl:choose>
						<xsl:when test="$cInventorFirstName=1 and $DisplayLastNameOnly!='true'">
							<xsl:choose>
								<xsl:when test="contains(b:First,$prop_APA_FromToDash)">
									<xsl:call-template name="HandleSPaceHypenInAuthor">
										<xsl:with-param name="author">
							...
							<xsl:choose>
								...
								<xsl:when test ="position()=$cInventors and $cInventorMiddleName=0">
									<!--<xsl:call-template name ="templ_prop_ListSeparator"/>-->
								</xsl:when>
								<xsl:when test ="$cInventorMiddleName!=0">
									<xsl:call-template name ="templ_prop_Space"/>
					...
					<xsl:choose>
						<xsl:when test="$cInventorMiddleName=1 and $DisplayLastNameOnly!='true'">
							<xsl:choose>
								<xsl:when test="contains(b:Middle,$prop_APA_FromToDash)">
									<xsl:call-template name="HandleSPaceHypenInAuthor">
										<xsl:with-param name="author">
							...
							<xsl:choose>
								...
								<xsl:when test ="position()=$cInventors">
									<!--<xsl:call-template name="templ_prop_Space"/>-->
								</xsl:when>
								<xsl:otherwise>
									<xsl:call-template name ="templ_prop_ListSeparator"/>
					...
					<xsl:if test="position()=1">
						...
						<xsl:choose>
							<xsl:when test="$cInventorLastName=1">
								<xsl:value-of select="b:Last"/>
								<xsl:choose>
									<xsl:when test="$DisplayLastNameOnly!='true'">
										<xsl:call-template name ="templ_prop_ListSeparator"/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:call-template name ="templ_prop_Space"/>
		...
		<xsl:choose>
			...
			<xsl:when test="$cAuthors>0 and 5>$cAuthors">
				<xsl:for-each select="b:Author/b:Author/b:NameList/b:Person">
					...
					<xsl:choose>
						<xsl:when test="$cAuthorLastName=1">
							...
							<xsl:choose>
								...
								<xsl:when test ="$cAuthorFirstName=0 and position()=$cAuthors">
									<xsl:call-template name="templ_prop_Space"/>
								</xsl:when>
								<xsl:when test ="$DisplayLastNameOnly and position()=$cAuthors">
									<xsl:call-template name="templ_prop_Space"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:call-template name ="templ_prop_ListSeparator"/>
					...
					<xsl:choose>
						<xsl:when test="$cAuthorFirstName=1 and $DisplayLastNameOnly!='true'">
							<xsl:choose>
								<xsl:when test="contains(b:First,$prop_APA_FromToDash)">
									<xsl:call-template name="HandleSPaceHypenInAuthor">
										<xsl:with-param name="author">
							...
							<xsl:choose>
								...
								<xsl:when test ="position()=$cAuthors and $cAuthorMiddleName=0">
									<!--<xsl:call-template name="templ_prop_Space"/>-->
								</xsl:when>
					...
					<xsl:choose>
						<xsl:when test="$cAuthorMiddleName=1 and $DisplayLastNameOnly!='true'">
							<xsl:choose>
								<xsl:when test="contains(b:Middle,$prop_APA_FromToDash)">
									<xsl:call-template name="HandleSPaceHypenInAuthor">
										<xsl:with-param name="author">
	...
	<xsl:template name ="BibDisplayVolumeIssue">
		...
		<xsl:if test ="$cPeriodicalTitle!=0">
			...
			<xsl:choose>
				<xsl:when test="$cYear!=0 or $cMonth!=0 or $cVolume!=0 or $cIssue!=0 or $cPages!=0">
					<xsl:call-template name ="templ_prop_ListSeparator"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:call-template name = "templ_prop_Dot"/>
				...
				<xsl:when test="$SourceType = 'Book'">
					<xsl:element name="p">
						...
						<xsl:call-template name = "BibDisplayAuthor">
							...
							<xsl:with-param name="DisplayLastNameOnly" select="'true'" />
						</xsl:call-template>
						<xsl:call-template name ="templ_prop_Space"/>
						<xsl:call-template name ="templ_prop_OpenBracket"/>
	...
	<xsl:template name="Citation">
		<xsl:for-each select="b:Citation">
			...
			<xsl:choose>
				<xsl:when test="$SourceType = 'Book' or                 $SourceType = 'BookSection' or
                ...
                $SourceType = 'ConferenceProceedings' ">
					<html 
						...
						<body>
							...
							<xsl:variable name ="pages">
								<xsl:choose>
									<xsl:when test="contains($initValueOfPages, '-')">
										<xsl:value-of select="concat('Seiten ', $initValueOfPages)"/>
									</xsl:when>
									<xsl:when test="contains($initValueOfPages, ',')">
										<xsl:value-of select="concat('Seiten ', $initValueOfPages)"/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="concat('Seite ', $initValueOfPages)"/>
									</xsl:otherwise>
								</xsl:choose>
							</xsl:variable>
							...
							<!--<xsl:if test="b:FirstAuthor"><xsl:call-template name ="templ_prop_OpenBracket"/></xsl:if>-->
							...
							<!--<xsl:if test="b:LastAuthor"><xsl:call-template name="templ_prop_CloseBracket"/></xsl:if>-->
	...
	<xsl:template name="formatNameCore">
		...
		<xsl:variable name="format">
			<xsl:choose>
				<xsl:when test="string-length($first) = 0 and string-length($middle) = 0 and string-length($last) = 0 "></xsl:when>
	...
	<xsl:template name="formatPersonSeperator">
		<xsl:choose>
			...
			<xsl:when test="count(../b:Person) > 3 and position() >= 2"></xsl:when>
